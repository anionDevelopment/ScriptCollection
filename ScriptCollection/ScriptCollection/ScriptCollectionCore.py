...
""")
        ...
        # remark: the content of the folders is passed to tar as "." and not as "*" because the programs are started without a shell, so a wildcard would not be expanded and would be treated as a literal filename
...
- [QualityCheck-system](TXDX)""".replace("XDX", "ODO").split("\n"))
    ...
    @GeneralUtilities.check_arguments
    def format_json_file(self, file: str, indentation: int = 2) -> None:
        """Reformats the given json-file with the given indentation. The order of the properties is kept
        because in json-files which are used as configuration the order is usually chosen intentionally."""
        encoding = "utf-8"
...
{ocr_content}""")
    ...
    def add_tooltips_to_chart_diagram(self, svg_file: str) -> int:
        """Adds a title-element to each data-point of a chart-diagram which was generated by generate_chart_diagram
        and returns the amount of data-points which were processed. Svg-viewers which support user-interaction (for
        example browsers) show the content of a title-element as tooltip while the mouse hovers over the according
        element. This is required because the tooltips which are defined in a vega-lite-specification are only
        available when the diagram is rendered by the vega-runtime inside a website, while the generated svg-file
        is static and does not contain that runtime. The content of the tooltip is the accessibility-label which
        vega already generates for the data-point and which contains the values of all encoded fields.
        Only data-points of symbol-marks are processed; for other mark-types (for example bars) the according
        role-description would have to be added."""
        encoding = "utf-8"
        svg_namespace = "http://www.w3.org/2000/svg"
        root = ET.XML(GeneralUtilities.read_text_from_file(svg_file, encoding))
        result = 0
        for element in root.iter():
            if element.get("aria-roledescription") != "point":
                continue
            ...
            title.text = label
            element.insert(0, title)#the title-element must be the first child-element of the element it describes
            result = result+1
        GeneralUtilities.write_text_to_file(svg_file, ET.tostring(root, encoding="unicode"), encoding)
        return result
