...
class TFCPS_CodeUnitSpecific_Base(ABC):
    ...
    def generate_reference_using_docfx(self, generate_class_reference:bool=False):
        ...
        toc_md_file:str=os.path.join(class_reference_folder,"toc.md")
        GeneralUtilities.ensure_file_does_not_exist(toc_md_file)#should not be available when docfx is runninng because then toc.html will be generated from toc.md but toc.html will be generated by docfx in the target-folder

        self.tfcps_Tools_General.generate_svg_files_from_plantuml_files_for_codeunit(self.get_codeunit_folder(), self.use_cache())
        ...
        if generate_class_reference:# this codeunit has a class-reference, so a human-readable table-of-content can be generated for it
            GeneralUtilities.ensure_file_exists(toc_md_file)
            GeneralUtilities.write_text_to_file(toc_md_file, self.__generate_toc_md_file_content(os.path.join(class_reference_folder,"toc.yml")))

    @staticmethod
    @GeneralUtilities.check_arguments
    def __generate_toc_md_file_content(toc_file:str)->str:
        """Generates the content of a markdown-table-of-content which contains a headline and links to the sub-files of the class-reference
        based on the given toc.yml-file which was generated by docfx."""
        toc_file_content = yaml.safe_load(GeneralUtilities.read_text_from_file(toc_file))
        namespaces:list = list()
        if isinstance(toc_file_content, dict):
            namespaces = toc_file_content.get("items", list())
        elif isinstance(toc_file_content, list):# older docfx-versions do not wrap the entries in an "items"-property
            namespaces = toc_file_content
        lines:list[str] = ["# Table of contents"]
        for namespace in namespaces:
            lines.append(GeneralUtilities.empty_string)
            lines.append(f"## {TFCPS_CodeUnitSpecific_Base.__get_toc_entry_name(namespace)}")
            lines.append(GeneralUtilities.empty_string)
            entries:list = namespace.get("items", list())
            if len(entries) == 0:
                lines.append("(This namespace does not contain any documented type.)")
            else:
                for entry in entries:
                    lines.append(f"- [{TFCPS_CodeUnitSpecific_Base.__get_toc_entry_name(entry)}]({TFCPS_CodeUnitSpecific_Base.__get_toc_entry_link(entry)})")
        return "\n".join(lines)+"\n"

    @staticmethod
    @GeneralUtilities.check_arguments
    def __get_toc_entry_name(entry:dict)->str:
        name:str = entry.get("name", entry.get("uid", GeneralUtilities.empty_string))
        return name.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")# generic types like "List<T>" would be treated as html-tag otherwise

    @staticmethod
    @GeneralUtilities.check_arguments
    def __get_toc_entry_link(entry:dict)->str:
        uid:str = entry["uid"]
        return "./"+uid.replace("`", "-")+".yml"# docfx replaces the backtick of generic types by a hyphen when generating the filenames
