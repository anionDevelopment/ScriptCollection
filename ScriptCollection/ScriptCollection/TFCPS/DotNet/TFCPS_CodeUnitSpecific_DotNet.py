...
<\\/Project>\\n?$"""
    ...
    def run_testcases(self) -> None:
        ...
        if not self.__coverage_file_contains_sources_and_packages(target_file):
            self._protected_sc.log.log("Coverage generated by XPlat collector is empty. Falling back to coverlet.msbuild coverage generation.")
            GeneralUtilities.ensure_file_does_not_exist(target_file)
            fallback_output = os.path.join(coverage_file_folder, "Testcoverage")
            fallback_args: list[str] = ["test", ".", "-c", dotnet_build_configuration]
            if os.path.isfile(os.path.join(codeunit_folder, runsettings_file)):
                fallback_args += ["--settings", runsettings_file]
            fallback_args += ["/p:CollectCoverage=true", "/p:CoverletOutputFormat=cobertura", f"/p:CoverletOutput={fallback_output}"]
            self._protected_sc.run_program_argsasarray("dotnet", fallback_args, codeunit_folder, print_live_output=self.get_verbosity()==LogLevel.Debug, timeoutInSeconds=60*20)
            fallback_file = f"{fallback_output}.cobertura.xml"
            GeneralUtilities.ensure_file_exists(fallback_file)
