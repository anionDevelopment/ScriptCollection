...
class TFCPS_CodeUnit_BuildCodeUnits:
    ...
    def search_for_secrets(self) -> None:
        ...
        if os.path.isdir(os.path.join(self.repository, ".git")):
            scan_args = ["git", "/repo"]
        elif os.path.isfile(os.path.join(self.repository, ".git")):
            ...
            scan_args = ["dir", "/repo"]
        # "-v" makes Betterleaks print each finding with file path, line and matched snippet
        # (otherwise only the summary "leaks found: N" is emitted and findings stay invisible).
        scan_args = scan_args + ["-v"]
